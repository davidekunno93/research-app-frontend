...
type SubjectsPageProps = {
    study: Study
    openRightSidePanel: (subjectId: string, subjectVisit: SubjectVisit, studyVisit: StudyVisit) => void
}

const SubjectsPage = ({ study, openRightSidePanel }: SubjectsPageProps) => {
    // [imports]
    const { textFunctions, timeFunctions } = useContext(DataContext);
                                            ...
                                            return <div key={index} className="col">
                                                ...
                                                <div onClick={() => visitFunctions.toggleVisitOptions(id)} id={`visit-optionsBtn-${id}`} className="options-btn">
                                                    ...
                                                    <div id={`visit-optionsDropper-${id}`} className="dropdown hidden" style={{ width: subjectVisit.icfSigned ? 180 : 150 }}>
                                                        <div onClick={() => openRightSidePanel(subject.id, subjectVisit, studyVisit)} className="option">
